...
// flow: matrix to be filled with edge flows (must start zeroed)
// s: source
// t: sink
// returns: max flow
    ...
    while(true) {
        ...
        while(!q.empty()) {
            ...
            for(int next = 0; next < cap.size(); ++next){
                if(cap[cur][next] - flow[cur][next] <= 0 || p[next] != -1)
                    continue;
        ...
        if(p[t] == -1) {
            ...
            int cur = t;
            while(p[cur] != cur) {
                bottleneck = min(bottleneck, cap[p[cur]][cur] - flow[p[cur]][cur]);
                cur = p[cur];
            }
