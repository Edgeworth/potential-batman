...
// protip: remember that input strings might be blank, so
// read them in with getline rather than stream extraction

enum { REP = 0, ADD, REM };

// ts     : a 2d vector of size u.size() + 1 by v.size() + 1
//          during execution of this function, the (i,j)th
//          entry of ts is set to the optimum operation used
//          to transform the length i prefix of u into the
...
void print_transforms(const string u, const string v,
                      const vvi& ts) {
