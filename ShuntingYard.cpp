...
int solve_calculator_language() {
	...
	while (!cin.eof()) {
		...
		// pad all operators with spaces and replace underscores with neg signs.
		string ll;
		for (char c : l) {
			if (ops.count(c) || c == '(' || c == ')') {
				ll.push_back(' ');
		...
		stack<pair<int, int>> s; // pair is <var, val>: var==-1 means a number stored in val
		for (string t : post) {
			if (is_op(t)) {
				int rhs = s.top().first >= 0 ? vars[s.top().first] : s.top().second; s.pop();
				int lhs_v = s.top().first;
				int lhs = s.top().first >= 0 ? vars[s.top().first] : s.top().second; s.pop();
				switch(t[0]) {
